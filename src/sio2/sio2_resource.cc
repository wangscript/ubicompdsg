...
void mysio2ResourceDispatchEvents( SIO2resource  *_SIO2resource,
								SIO2window	   *_SIO2window,
								  unsigned char	 _type,
								  unsigned char	 _state,
								  float           _deltaScale,
								  ...
								  float           _detZ )
{
	if( _SIO2resource )
	{
		...
		switch( _type )
		{
			...
			//added by danielbas{
			case my_WINDOW_CHANGE_OBJ_SCALE:
			{
				if( _SIO2window->_SIO2windowChangeObjScl )
				{ _SIO2window->_SIO2windowChangeObjScl( _SIO2window ,_deltaScale ); }
				break;
			}
			case my_WINDOW_ROTATE_OBJ:
			{
				if( _SIO2window->_SIO2windowRotateObj )
				{ _SIO2window->_SIO2windowRotateObj( _SIO2window , _rotateDirection, _DirectionState ); }
				break;
			}
			case my_WINDOW_MOVE_OBJ:
			{
				if( _SIO2window->_SIO2windowMoveObj )
				{ _SIO2window->_SIO2windowMoveObj( _SIO2window , _detX, _detY, _detZ ); }
				break;
			}
			case my_WINDOW_MOVE_CAMERA:
			{
				if( _SIO2window->_SIO2windowMoveCamera )
				{ _SIO2window->_SIO2windowMoveCamera( _SIO2window , _detX, _detY, _detZ ); }
				break;
			}
			case my_WINDOW_BACK_HANDLE:
				if( _SIO2window->_SIO2windowBackHandle )
				{ _SIO2window->_SIO2windowBackHandle( _SIO2window , _rotateDirection, _DirectionState, _detX, _detY ); }
				break;
