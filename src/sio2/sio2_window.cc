...
void sio2WindowAddTouch( SIO2window *_SIO2window,
						float		 _x,
						float		 _y )
{
	++_SIO2window->n_touch;
	
	_SIO2window->touch = ( vec2 ** ) realloc( _SIO2window->touch,
											 _SIO2window->n_touch * sizeof( vec2 ) );
	
	_SIO2window->touch[ _SIO2window->n_touch - 1 ] = sio2Vec2Init();
	_SIO2window->touch[ _SIO2window->n_touch - 1 ]->x = _y;
	_SIO2window->touch[ _SIO2window->n_touch - 1 ]->y = _x;

}


void sio2WindowResetTouch( SIO2window *_SIO2window )
{
	if( _SIO2window->touch )
	{
